...
export function AutoCompleteLocation({ navigation }){
    ...
    const renderItems= filteredItems.map(item=>{
       return <TouchableOpacity onPress={()=> navigation.navigate(path,{location:item})} key={item.id} style={styles.item}><Bold style={styles.itemText}>{item}</Bold></TouchableOpacity>
    })
