...
export default function Anteprima({ navigation }) {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <TouchableOpacity
        onPress={() => navigation.navigate('AutoCompleteLocation', { path: 'Dove' })}>
        {
          <View style={styles.locationContainer}>
            {passedComune ? (
              <LocationWithText
                color={'black'}
                comune={passedComune}
                regione={passedRegione}></LocationWithText>
            ) : (
              <Light>Imposta la località dove vuoi pubblicare l'annuncio</Light>
            )}
          </View>
        }
      </TouchableOpacity>
