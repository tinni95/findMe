...
export default function Requisiti({ navigation }) {
  const { requisiti, categoria, servizio } = navigation.state.params;
  ...
  return (
    <ScrollView style={styles.container}>
      <HeaderBarLeft onPress={() => navigation.navigate('Posizione')}></HeaderBarLeft>
      <HeaderTitle text={'Requisiti'}></HeaderTitle>
      <RequisitiMultiSelect
        active={active}
        setActive={setActive}
        items={requisiti}></RequisitiMultiSelect>
      {custom.map((custo) => {
        return (
          <TouchableOpacity onPress={() => setCustom(custom.filter((i) => i !== custo))}>
            <RequisitiPicker selected={true} text={custo}></RequisitiPicker>
          </TouchableOpacity>
        );
      })}
      ...
      <InputOverlayModal
        ...
        title={'Aggiungi requisito'}
        input={custom}
        setInput={setCustom}></InputOverlayModal>
      <View style={{ flex: 1, margin: 50, justifyContent: 'center', alignItems: 'center' }}>
        <RoundButton
          onPress={() =>
            navigation.navigate('Descrizione', { categoria, servizio, requisiti: parseRequisiti() })
          }
          text={'Procedi'}
          color={Colors.blue}
