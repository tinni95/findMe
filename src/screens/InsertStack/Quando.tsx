...
export default function Quando({ navigation, route }) {
  const { categoria, servizio, descrizione } = navigation.state.params;
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={{ margin: 20 }}>
        <SingleFilter
          inactive={false}
          settori={['In un giorno preciso', 'Entro una data', 'Da definire']}
          setItem={(item) => setQuando(item)}
          settoreAttivi={-1}
        />
        <View style={{ marginTop: 20 }} />
        {quando === 'Entro una data' && (
          <View>
            <LinearGradient
              start={[0, 1]}
              end={[1, 0]}
            ...
            <TouchableOpacity onPress={() => setVisibleDate(true)}>
              <FormTextInput
                ...
                style={dataError ? FormStyles.inputError : FormStyles.input}
              />
            </TouchableOpacity>
          </View>
        )}
        {quando === 'In un giorno preciso' && (
          <View>
            <LinearGradient
              start={[0, 1]}
              end={[1, 0]}
            ...
            <SingleFilter
              inactive={false}
              settori={['Da definire', 'Mattino', 'Pomeriggio', 'Sera']}
              setItem={(item) => setGiornata(item)}
              settoreAttivi={-1}
            />
            <View style={{ marginTop: 20 }} />
            <LinearGradient
