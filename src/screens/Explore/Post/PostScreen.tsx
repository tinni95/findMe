  ...
  const [deleteApplication] = useMutation(DELETEAPPLICATION_MUTATION, {
    ...
    onError: (error) => {
      if (error.toString().includes('Closed')) {
        alert(
          "L'applicazione è stata chiusa, non puoi rimuovere la tua applicazione in questo momento",
        );
      ...
      <View style={styles.descriptionCard}>
        ...
        <View style={styles.DesriptionContainer}>
          ...
          <Light style={styles.body}>
            {' '}
            {data.singlePost.data ? moment(data.singlePost.data).format('LL') : 'Da definire'}
          </Light>
        </View>
          ...
          <View style={styles.DesriptionContainer}>
            ...
            {data.singlePost.startTime || data.singlePost.endTime ? (
              <Light>
                <Light style={styles.body}>
                  {' '}
                  {data.singlePost.startTime && 'dalle ' + data.singlePost.startTime}{' '}
                </Light>
                <Light style={styles.body}>
                  {' '}
                  {data.singlePost.endTime && ' alle ' + data.singlePost.endTime}{' '}
                </Light>
              </Light>
            ) : (
        ...
        <View style={styles.ButtonWrapper}>
          {!isOwner && (
            <RoundButton
              onPress={() =>
                data.applicationUserForPosition.length > 0 ? handleRimuovi() : handleApply()
              }
              text={
                data.applicationUserForPosition.length > 0 ? 'Rimuovi Candidatura' : 'Candidati'
              }
              textColor={'white'}
              color={Colors.red}
            />
...
PostScreenWS.navigationOptions = ({ navigation }) => {
  return {
    ...
    headerRight: () =>
      navigation.getParam('isOwner', false) && (
        <HeaderRightElimina onPress={() => navigation.getParam('deleteP', null)} />
      ),
  };
};
