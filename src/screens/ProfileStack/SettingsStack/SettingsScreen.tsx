...
function SettingsScreen({ navigation, route, context }) {
  ...
  const [deleteUser] = useMutation(DELETE_USER_MUTATION, {
    ...
    onError: () => {
      alert('oops... si è verificato un errore');
    },
  });
  const [sendEmail] = useMutation(SEND_EMAIL_MUTATION, {
    onCompleted: () => {
      alert('email invitata');
    },
    onError: () => {
      alert('oops... si è verificato un errore');
    },
  ...
  return (
    <ScrollView
      ...
      />
      <SettingsButton
        onPress={() => navigation.navigate('UpdateEmail', { onGoBack: () => refetch() })}
        text={'Cambia e-mail'}
      />
      <View style={styles.spacer} />
      ...
      <SettingsButton
        ...
        text={'Privacy policy'}
      />
      <SettingsButton onPress={() => navigation.navigate('About')} text={'Riguardo Tendit'} />
      <View style={styles.spacer} />
      <Bold style={styles.sectionTitle}>ELIMINA ACCOUNT</Bold>
      <SettingsButton onPress={() => deleteAccount()} text={'Elimina account'} color={Colors.red} />
    </ScrollView>
  );
}
