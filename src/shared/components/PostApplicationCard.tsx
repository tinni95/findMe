...
export default function PostApplicationCard({ isRefetch, opened, id, navigation }) {
  ...
  if (loading) {
    ...
    return (
      <View style={[styles.container, { opacity: opened ? 1 : 0.6 }]}>
        <Bold style={styles.header}>{data.applicationsForPosition[0].post.titolo}</Bold>
        <Bold style={styles.subHeader}>{data.applicationsForPosition.length} risposte</Bold>
            ...
            if (index < 4) {
              return (
                <Image
                  key={post.id}
                  style={styles.image}
                  source={getUri(post.from.pictureUrl)}></Image>
              );
            }
          })}
        </View>
