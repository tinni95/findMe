...
export function PositionCard({ post, position, button, navigation }) {
  ...
  const { loading, data, refetch } = useQuery(application, {
    variables: {
      id: position.id,
    },
            ...
            {position.requisiti ? (
              <View style={styles.DescriptionItem}>
                ...
                {position.requisiti.length == 0 ? (
                  <Light style={styles.DescriptionItemBody}>Non Specificato</Light>
                ) : (
                  position.requisiti.map((requisito) => {
                    return (
                      <Light key={shortid.generate()} style={styles.DescriptionItemBody}>
                        - {requisito}
                      </Light>
                    );
                  })
